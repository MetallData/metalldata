...
namespace
{
  ...
  toValueExpr(experimental::dataframe_variant_t el)
  {
    if (const experimental::string_t* s = std::get_if<experimental::string_t>(&el))
                 ...
                 )
  {
    ...
    forAllSelected( [&res](int rownum) -> void { res.push_back(rownum); },
                    rank,
                    dataset,
  ...
  getSelectedRows(int rank, const clippy::clippy& clip, const vector_json_type& vec, int numrows = std::numeric_limits<int>::max())
  {
    ...
    {
      CXX_UNLIKELY;
      return generateIndexN(std::vector<int>{}, std::min(numrows, int(vec.rows())));
    }
 ...
 *
  boost::json::value
  projectJsonEntry(const vector_json_type& df, const ColumnSelector& projlst, int idx)
  {
