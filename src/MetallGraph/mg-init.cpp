...
int ygm_main(ygm::comm& world, int argc, char** argv)
{
  ...
  // the real thing
    ...
    const ARG_EDGE_SRCKEY_TYPE edgeSrcKey   = clip.get<ARG_EDGE_SRCKEY_TYPE>  (ARG_EDGE_SRCKEY_NAME);
    const ARG_EDGE_DSTKEY_TYPE edgeDstKey   = clip.get<ARG_EDGE_DSTKEY_TYPE>  (ARG_EDGE_DSTKEY_NAME);
    ...
    if (!std::filesystem::is_directory(dataLocation))
    {
      ...
      if (vertexKey.size() == 0)  throw std::runtime_error{missingKeyA + ARG_VERTEX_KEY_NAME  + missingKeyZ};
      if (edgeSrcKey.size() == 0) throw std::runtime_error{missingKeyA + ARG_EDGE_SRCKEY_NAME + missingKeyZ};
      if (edgeDstKey.size() == 0) throw std::runtime_error{missingKeyA + ARG_EDGE_DSTKEY_NAME + missingKeyZ};
