...
const std::string ARG_COLUMN_DESC = "columns";
}  // namespace
...
void appendColumn(xpr::DataFrame& df, const ColumnDescription& desc) {
  ...
  if (type(desc) == "uint")
    ...
    df.add_column_default_value(
        xpr::dense<xpr::string_t>{df.persistent_string("")});
  else
...
void appendColumns(xpr::DataFrame&                       df,
                   const std::vector<ColumnDescription>& cols) {
  for (const ColumnDescription& desc : cols) appendColumn(df, desc);
}
...
int ygm_main(ygm::comm& world, int argc, char** argv) {
  ...
  clip.add_optional<std::string>(
      ST_METALLFRAME_NAME,
      "Name of the metallframe object within the Metall store", "_default");
  ...
  try {
    ...
    std::unique_ptr<xpr::DataFrame> dfp =
        makeDataFrame(createNew, location, key);
