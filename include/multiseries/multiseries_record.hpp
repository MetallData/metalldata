...
/// Each series can have different types. Supported types are int64_t, uint64_t, double, and std::string_view.
...
class basic_record_store {
  ...
  public:
    ...
    series_info_type<series_type> add_series(std::string_view series_name,
      ...
      m_series.push_back(
        {
          .name = string_type(series_name.data(),
                              series_name.size(),
    ...
    template<typename series_type>
    void set(std::string_view series_name,
    ...
    void for_all(std::string_view series_name,
                 series_func_t series_func) const {
      auto itr = priv_find_series(series_name);
      if (itr == m_series.end()) {
        throw std::runtime_error("Series not found");
    ...
    bool remove_record(const record_id_type record_id) {
      ...
      for (auto &series : m_series) {
        std::visit(
          [&record_id](auto &container) { container.erase(record_id); },
          series.container);
      }
    ...
    void convert(std::string_view series_name,
                 container_kind new_kind) {
      auto itr = priv_find_series(series_name);
      if (itr == m_series.end()) {
        throw std::runtime_error("Series not found");
      }
  ...
  /// When the series container is dense, it returns the ratio of the number of items to the capacity.
    double load_factor(std::string_view series_name) const {
      ...
      return std::visit(
        [](const auto &container) { return container.load_factor(); },
        itr->container);
    }
  ...
  private:
    ...
    const auto priv_get_series_data(const container_variant &series_store,
      ...
      if constexpr (std::is_same_v<series_type, std::string_view>) {
        ...
        return priv_get_series_container<series_type>(series_store).at(
          record_id);
      }
    }
