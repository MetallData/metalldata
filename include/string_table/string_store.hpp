...
#if USE_BOOST_UNORDERED_FLAT_CONTAINER
...
template <typename Alloc = std::allocator<std::byte>> class string_store {
private:
...
#if USE_BOOST_UNORDERED_FLAT_CONTAINER
  using set_type =
      boost::unordered_flat_set<str_holder, set_hasher, std::equal_to<>,
                                set_allocator_type>;
#else
  using set_type = boost::unordered_set<str_holder, set_hasher, std::equal_to<>,
                                        set_allocator_type>;
#endif
...
public:
  ...
  const char *find_or_add(std::string_view str) {
    auto itr =
        m_set.find(str, boost::hash<std::string_view>(), str_holder_equal());
    if (itr != m_set.end()) {
...
string_accessor add_string(const char *const str, const std::size_t len,
