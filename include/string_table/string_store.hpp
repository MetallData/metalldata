...
char *allocate_string_embedding_length(const std::string_view &str,
  ...
  std::char_traits<char>::copy(str_buf, std::to_address(str.data()),
                               str.size());
...
class string_store {
 ...
 private:
  ...
  class str_holder {
   public:
    ...
    // not equal operator
    bool operator!=(const str_holder &other) const { return !(*this == other); }
 ...
 private:
  ...
  void priv_deallocate_string(const std::string_view &str) {
    static_assert(
      std::is_same_v<typename std::allocator_traits<allocator_type>::value_type,
                     char>,
...
string_accessor add_string(std::string_view sv, string_store<Alloc> &store) {
  // If string is short, store it in the holder
  // otherwise, store it in the store and store the pointer in the holder
  if (sv.length() <= string_accessor::short_str_max_length()) {
    return string_accessor(sv.data(), sv.length());
  }
  return string_accessor(store.find_or_add(sv.data()), sv.length());
}

/// \brief Helper function to find a string to the string store.
/// \warning This only truly 'finds' long strings because short strings
/// are encoded directly in the string_accessor.
template <typename Alloc>
std::optional<string_accessor> find_string(std::string_view           sv,
                                           const string_store<Alloc> &store) {
  // If string is short, store it in the holder
  // otherwise, store it in the store and store the pointer in the holder
  if (sv.length() <= string_accessor::short_str_max_length()) {
    return string_accessor(sv.data(), sv.length());
  }
  const char *ptr = store.find(sv);
  if (ptr) {
