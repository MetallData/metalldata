 ...
 public:
  ...
  int run(ygm::comm& comm) override {
    if (hash_key.has_value()) {
      return run_distributed(comm);
    }
    return run_local(comm);
  }

  int run_distributed(ygm::comm& comm) {
    ...
    auto* pm_hash_key = manager.construct<persistent_string>("hash_key")(
        hash_key_unwrapped.c_str(), manager.get_allocator());
  ...
  int run_local(ygm::comm& comm) {
    comm.cout0("Local ingest from: ", input_path, " into ", metall_path,
               " recursive: ", recursive);
    ygm::utility::timer                 setup_timer;
    metall::utility::metall_mpi_adaptor mpi_adaptor(
        metall::create_only, metall_path, comm.get_mpi_comm());
